      ...
      versions: [
        {
          v: "v1",
          components: [
            { name: "Basic", type: "Earning", calc: "Fixed", value: "basic" },
            { name: "HRA", type: "Earning", calc: "%basic", value: 20 },
            { name: "PF", type: "Deduction", calc: "%basic", value: 12 },
          ],
        },
      ],
    },
...
const api = {
  async fetchEmployees() {
    await delay();
    return mockDB.employees.slice();
  },
  async fetchStructures() {
    await delay();
    return mockDB.structures.slice();
  },
  async fetchAssignments({ filter = {} } = {}) {
    ...
    if (filter.employee)
      list = list.filter((a) =>
        a.employeeName.toLowerCase().includes(filter.employee.toLowerCase())
      );
    return list;
  },
  async fetchAssignmentById(id) {
    await delay();
  ...
  async saveAssignment(payload) {
    ...
    if (!payload.id) {
      ...
      if (idx >= 0) {
        mockDB.assignments[idx] = { ...payload };
        return { ok: true, assignment: mockDB.assignments[idx] };
      }
...
function computeComponents(
  ...
  const ver =
    (structure?.versions || []).find((v) => v.v === version) ||
  ...
  additionalItems.forEach((ai) => {
    if (ai.occurrence === "Monthly")
      comps.push({
        name: ai.name,
        type: ai.type === "Earning" ? "Earning" : "Deduction",
        value: ai.amount,
      });
  });
  const monthlyTotal = comps.reduce(
    (s, c) => s + (c.type === "Earning" ? c.value : -c.value),
    0
  );
...
function AssignmentModal({
  ...
  function validate() {
    ...
    if (!form.baseSalary || Number(form.baseSalary) <= 0)
      e.baseSalary = "Base salary must be > 0";
  ...
  function addAdditional() {
    setForm((f) => ({
      ...f,
      additionalItems: f.additionalItems.concat({
        id: Date.now(),
        name: "",
        type: "Earning",
        amount: 0,
  ...
  function removeAdditional(idx) {
    setForm((f) => ({
      ...f,
      additionalItems: f.additionalItems.filter((_, i) => i !== idx),
    }));
  }
  function updateAdditional(idx, key, value) {
    setForm((f) => ({
      ...f,
      additionalItems: f.additionalItems.map((a, i) =>
        i === idx ? { ...a, [key]: value } : a
      ),
    }));
  }
  ...
  async function submit(e) {
    ...
    const change = {
      version: (form.history?.length || 0) + 1,
      effectiveFrom: form.effectiveFrom,
      effectiveTo: form.effectiveTo || null,
      changedBy: "You",
      summary: form.notes || "Assignment created/edited",
    };
  ...
  const { comps, monthlyTotal, annualCTC, netSalary } = computeComponents(
    selectedStructure,
    form.structureVersion,
    form.baseSalary,
    form.overrides,
      ...
      >
        ...
        <div className={styles.modalBody}>
          <div className={styles.leftCol}>
            ...
            <select
              ...
              onChange={(e) =>
                setForm((f) => ({
                  ...
                  structureVersion:
                    structures.find((s) => s.id === e.target.value)?.versions[0]
                      .v || "",
                }))
              }
            >
            ...
            {errors.structureId && (
              <div className={styles.fieldError}>{errors.structureId}</div>
            )}

            <label className={styles.label}>Version</label>
            <select
              className={styles.input}
              value={form.structureVersion}
              onChange={(e) =>
                setForm((f) => ({ ...f, structureVersion: e.target.value }))
              }
            >
              {(
                structures.find((s) => s.id === form.structureId)?.versions ||
                []
              ).map((v) => (
            ...
            />
            {errors.baseSalary && (
              <div className={styles.fieldError}>{errors.baseSalary}</div>
            )}

            <label className={styles.label}>
              <input
                type="checkbox"
                checked={form.calculateCTCFromBase}
                onChange={(e) =>
                  setForm((f) => ({
                    ...f,
                    calculateCTCFromBase: e.target.checked,
                  }))
                }
              />{" "}
              Calculate CTC from Base
            </label>
          ...
          <div className={styles.rightCol}>
            ...
            <div className={styles.previewBox}>
              <table className={styles.previewTable}>
                ...
                <tfoot>
                  ...
                  <tr>
                    <td colSpan="2">Annual CTC (est.)</td>
                    <td>{currency(annualCTC)}</td>
                  </tr>
                  <tr>
                    <td colSpan="2">Net Salary (est.)</td>
                    <td>{currency(netSalary)}</td>
                  </tr>
                </tfoot>
              </table>
            </div>
            ...
            <div>
              ...
              <div className={styles.overrideList}>
                {form.overrides.map((o, idx) => (
                  <div key={o.id} className={styles.overrideRow}>
                    <input
                      ...
                      value={o.name}
                      onChange={(e) =>
                        updateOverride(idx, "name", e.target.value)
                      }
                    />
                    <select
                      className={styles.inputSmall}
                      value={o.overrideType}
                      onChange={(e) =>
                        updateOverride(idx, "overrideType", e.target.value)
                      }
                    >
                ...
                {form.overrides.length === 0 && (
                  <div className={styles.muted}>No overrides</div>
                )}
              </div>
            </div>
            ...
            <div className={styles.overrideList}>
              {form.additionalItems.map((a, idx) => (
                <div key={a.id} className={styles.overrideRow}>
                  ...
                  <select
                    ...
                    value={a.type}
                    onChange={(e) =>
                      updateAdditional(idx, "type", e.target.value)
                    }
                  >
                  ...
                  <input
                    ...
                    value={a.amount}
                    onChange={(e) =>
                      updateAdditional(idx, "amount", Number(e.target.value))
                    }
                  />
                  <select
                    className={styles.inputSmall}
                    value={a.occurrence}
                    onChange={(e) =>
                      updateAdditional(idx, "occurrence", e.target.value)
                    }
                  >
              ...
              {form.additionalItems.length === 0 && (
                <div className={styles.muted}>No additional items</div>
              )}
            </div>
            ...
            <label className={styles.label}>
              <input
                type="checkbox"
                checked={form.statutory.PF}
                onChange={(e) =>
                  setForm((f) => ({
                    ...f,
                    statutory: { ...f.statutory, PF: e.target.checked },
                  }))
                }
              />{" "}
              PF enabled
            ...
            <label className={styles.label}>
              <input
                type="checkbox"
                checked={form.statutory.ESI}
                onChange={(e) =>
                  setForm((f) => ({
                    ...f,
                    statutory: { ...f.statutory, ESI: e.target.checked },
                  }))
                }
              />{" "}
              ESI enabled
            ...
            <input
              className={styles.input}
              value={form.statutory.taxRegion}
              onChange={(e) =>
                setForm((f) => ({
                  ...f,
                  statutory: { ...f.statutory, taxRegion: e.target.value },
                }))
              }
            />
            ...
            <select
              ...
              onChange={(e) =>
                setForm((f) => ({
                  ...f,
                  payment: { ...f.payment, method: e.target.value },
                }))
              }
            >
            ...
            <input
              ...
              onChange={(e) =>
                setForm((f) => ({
                  ...f,
                  payment: { ...f.payment, bankName: e.target.value },
                }))
              }
            />
            ...
            <input
              ...
              onChange={(e) =>
                setForm((f) => ({
                  ...f,
                  payment: { ...f.payment, accountNumber: e.target.value },
                }))
              }
            />
          </div>
...
function HistoryModal({ open, assignment, onClose }) {
    ...
    <div className={styles.historyBackdrop} onClick={onClose}>
      <div className={styles.historyModal} onClick={(e) => e.stopPropagation()}>
        <div className={styles.historyHeader}>
          <h3>Version History — {assignment.employeeName}</h3>
...
export default function PayrollEmployeeAssignment({ navigate }) {
  ...
  useEffect(() => {
    loadAll();
  }, [filterDept, filterStatus, filterEmployee]);

  function openNew() {
    setModalInitial(null);
    setModalOpen(true);
  }
  async function onSaveAssignment(a) {
    setToast("Saved assignment");
      ...
      <div className={styles.page}>
        <div className={styles.headerRow}>
          <div className={styles.headerActions}>
            <button className={"homebtn"} onClick={openNew}>
              Create Assignment
            </button>
            <button
              className={"homebtn"}
              onClick={() => alert("Bulk Assign (simulate)")}
            >
              Bulk Assign
            </button>
            <button
              className={"homebtn"}
              onClick={() => alert("Import (CSV/XLSX) (simulate)")}
            >
              Import Assignments
            </button>
            <button
              className={"homebtn"}
              onClick={() => alert("Export Assignments (simulate)")}
            >
              Export Assignments
            </button>
          </div>
          ...
          <div className={styles.filter}>
            <div>
              ...
              >
                ...
                {[...new Set(employees.map((e) => e.dept))].map((d) => (
              ...
              <table className={"square-table w-75"}>
                ...
                <tbody>
                  {assignments.length === 0 && (
                    <tr>
                      <td colSpan="10">No assignments</td>
                    </tr>
                  )}
                  {assignments.map((a) => {
                    const struct = structures.find(
                      (s) => s.id === a.structureId
                    );
                    const { annualCTC } = computeComponents(
                      struct,
                      a.structureVersion,
                      a.baseSalary,
                      a.overrides,
                    ...
                    return (
                      <tr key={a.id}>
                        ...
                        <td>{a.designation}</td>
                        <td>
                          {struct?.name || "—"} / {a.structureVersion}
                        </td>
                        ...
                        <td>
                          <button
                            ...
                            onClick={async () => {
                              const full = await api.fetchAssignmentById(a.id);
                        ...
                        <td>
                          <button
                            ...
                            onClick={async () => {
                              const res = await api.exportAssignmentPDF(a.id);
                              if (res.ok) {
                                setToast("Downloaded CTC (simulate)");
